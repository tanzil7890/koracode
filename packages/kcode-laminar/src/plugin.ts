...
// README and runs once on key application via the kcode-browser telemetry
// module, not here.
...
export const LaminarPlugin: Plugin = ({ client }) => {
  ...
  // spawn kcode as a subprocess and want the agent's traces to nest under
    ...
    event: async ({ event }) => {
      switch (event.type) {
        ...
        case "server.instance.disposed": {
          ...
          // both observed to fire mid-await in headless `kcode run` mode
