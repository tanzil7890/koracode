# @koracode/kcode-browser

Level-1 KoraCode package: substantial, self-contained code with zero upstream-fork friction.

See `decisions.md §1c` (three-level model) and `§1d` (this package) in the KoraCode project memory.
...
| `script/embed-skills.ts` | Build-time embed; emits `kcode-skills.gen.ts` consumed by the compiled binary. |
