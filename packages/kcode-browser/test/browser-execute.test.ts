...
// cdp-smoke.test.ts (KCODE_SMOKE_CHROME=1 + KCODE_SMOKE_PROFILE_DIR).
//
...
const profileDir = process.env.KCODE_SMOKE_PROFILE_DIR
const enabled = process.env.KCODE_SMOKE_CHROME === "1" && profileDir
...
beforeAll(async () => {
  workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-be-"))
  dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-data-"))
})
...
test.skipIf(!enabled)("workspace import inside a snippet", async () => {
  ...
  await fs.writeFile(
    ...
    `export const run = async (session) => {
       ...
       await session.Page.navigate({ url: "data:text/html,<title>kcode-be</title>" })
       await loaded
  ...
  expect(result.output).toContain("title: kcode-be")
  expect(JSON.parse(result.result)).toBe("kcode-be")
})
...
test.skipIf(!enabled)("KCODE_SCREENSHOT_DIR dumps screenshots to disk", async () => {
  const dump = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-shotdump-"))
  const prev = process.env.KCODE_SCREENSHOT_DIR
  process.env.KCODE_SCREENSHOT_DIR = dump
  try {
    ...
    if (prev === undefined) delete process.env.KCODE_SCREENSHOT_DIR
    else process.env.KCODE_SCREENSHOT_DIR = prev
...
test("console.debug is captured; uncommon methods fall through without throwing", async () => {
  const data = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-debug-"))
  const ws = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-debug-ws-"))
  const result = await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
...
const runTimeout = async (id: string, code: string, timeout: number, onChunk?: (o: string) => Effect.Effect<void>) => {
  const data = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-to-"))
  const ws = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-to-ws-"))
...
test("re-running the execute effect after a timeout gets fresh state", async () => {
  const data = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-rerun-"))
  const ws = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-rerun-ws-"))
...
test("overlapping execute calls do not clobber each other's console capture", async () => {
  ...
  const aWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-conc-a-"))
  const bWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-conc-b-"))
  const aData = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-data-a-"))
  const bData = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-data-b-"))
