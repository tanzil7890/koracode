...
test("resolveSkillsDir materializes skills with {{SKILLS_DIR}} substituted", async () => {
  const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-skills-"))
  try {
...
test("different dataDirs get their own substituted paths", async () => {
  const a = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-skills-a-"))
  const b = await fs.mkdtemp(path.join(os.tmpdir(), "kcode-skills-b-"))
  try {
