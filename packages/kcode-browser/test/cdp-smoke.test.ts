...
// `KCODE_SMOKE_CHROME=1` and a running Chrome with `--remote-debugging-port`
...
test.skipIf(!enabled)("Session connects, navigates, reads title", async () => {
  ...
  try {
    ...
    await session.domains.Page.navigate({ url: "data:text/html,<title>kcode-smoke</title>" })
    await loaded
    ...
    expect(r.result.value).toBe("kcode-smoke")
  } finally {
    session.close()
  }
})
