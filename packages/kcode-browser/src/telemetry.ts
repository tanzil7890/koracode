...
// `KCODE_DEFAULT_LMNR_KEY` with a string literal via Bun's `define`. The
...
//
// No `if (telemetryEnabled)` branches downstream — `@koracode/kcode-laminar`
...
declare const KCODE_DEFAULT_LMNR_KEY: string
...
export const applyTelemetryKey = () => {
  ...
  if (typeof KCODE_DEFAULT_LMNR_KEY === "undefined" || !KCODE_DEFAULT_LMNR_KEY) return
  process.env.LMNR_PROJECT_API_KEY = KCODE_DEFAULT_LMNR_KEY
...
const showFirstRunNoticeOnce = () => {
  const marker = join(stateDir(), "kcode", "telemetry-notice-shown")
  if (Bun.file(marker).size > 0) return
  process.stderr.write(
    "KoraCode sends anonymous usage traces. Set DO_NOT_TRACK=1 to opt out.\n",
  )
  try {
