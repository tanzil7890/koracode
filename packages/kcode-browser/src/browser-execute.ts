...
// owns under `<projectDir>/.kcode/agent-workspace/`. Same mechanism for a
...
// clobber. See `memory/koracode/phase_h_eval_feasibility_findings.md`
...
export interface ExecuteContext {
  ...
  // Per-project workspace dir: <projectDir>/.kcode/agent-workspace/. Created
...
//
// `dataDir` is opencode's XDG_DATA_HOME for kcode (~/.local/share/kcode/ on
// Linux/Mac). Compiled-mode skills are extracted to `<dataDir>/skills/` once
// per build hash; dev mode resolves to the in-tree `packages/kcode-browser/
...
export const make = Effect.fn("BrowserExecute.make")(function* (dataDir: string) {
  ...
  const execute = (args: Parameters, ctx: ExecuteContext) =>
    ...
    return Effect.gen(function* () {
      ...
      // when `KCODE_SCREENSHOT_DIR` is set, also written to disk for
      ...
      const dumpDir = process.env.KCODE_SCREENSHOT_DIR
...
async function ensureCloudConnected(sessionID: string, session: ReturnType<typeof SessionStore.get>) {
  ...
  try {
    ...
    // One automatic attempt per logical KoraCode session. A later disconnect
