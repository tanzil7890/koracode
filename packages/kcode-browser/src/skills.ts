...
// `<target>/.kcode-build` recording `<buildHash>:<target>`. When it matches
...
const materialize = async (target: string): Promise<string> => {
  ...
  const embed = isCompiled ? await import("kcode-skills.gen.ts").catch(() => null) : null
  if (isCompiled && !embed) throw new Error("kcode-skills.gen.ts not found — was the build script updated?")
