...
const TIPS: Tip[] = [
  ...
  "Create {highlight}kcode.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/kcode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.kcode/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.kcode/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.kcode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.kcode/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent KoraCode from reading sensitive files",
  "Use {highlight}kcode run{/highlight} for non-interactive scripting",
  "Use {highlight}kcode --continue{/highlight} to resume the last session",
  "Use {highlight}kcode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}kcode serve{/highlight} for headless API access to KoraCode",
  "Use {highlight}kcode run --attach{/highlight} to connect to a running server",
  "Run {highlight}kcode upgrade{/highlight} to update to the latest version",
  "Run {highlight}kcode auth list{/highlight} to see all configured providers",
  "Run {highlight}kcode agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.kcode/themes/{/highlight} directory",
  ...
  "Run {highlight}kcode debug config{/highlight} to troubleshoot configuration",
