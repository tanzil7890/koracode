#!/usr/bin/env bun
//
// Builds the `kcode-<target>-serve` binary variant: registers only `kcode serve`
...
for (const target of targets) {
  const asset = `kcode-${target}-serve`
  const bin = path.join(dir, "dist", asset, "bin/kcode")
  ...
  const built = await Bun.build({
    ...
    entrypoints: ["./src/index.ts", "kcode-skills.gen.ts", "opencode-web-ui.gen.ts"],
    ...
    define: {
      ...
      // @koracode/kcode-browser/src/telemetry.ts.
