// Serve-only entrypoint for the `kcode-<target>-serve` binary variant.
//
// `packages/opencode/src/index.ts` eagerly imports all 24 command modules.
// Headless containers only ever invoke `kcode serve`, so registering just that
...
import { ServeCommand } from "@koracode/koracode-core/cli/cmd/serve"
import { UI } from "@koracode/koracode-core/cli/ui"
import { FormatError } from "@koracode/koracode-core/cli/error"
import { errorMessage } from "@koracode/koracode-core/util/error"
import { Heap } from "@koracode/koracode-core/cli/heap"
...
try {
  ...
  // Single drain point for OTel-based plugins (e.g. kcode-laminar); without it
  // trailing spans are lost. Mirrors the drain in packages/opencode/src/index.ts.
  try {
    const { pluginShutdownHooks } = await import("@koracode/koracode-core/plugin/index")
    await Promise.race([
      Promise.allSettled(
